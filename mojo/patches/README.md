...
changes (F7, F8 and the two rows below them) are on the branch as well.
...
| —     | `tools/kgen/kgen.cpp`                                                                                                                                                                                                                        | `--emit-quantum-kernels=DIR`: write each exported quantum kernel, plus everything it calls, as a generic-form module with locations. This is what qcc reads until the compiler invokes qcc itself (F9).                                                                                                                                |
