...
changes (F7, F8 and the two rows below them), F9, and phase 3's F12 and F13
are on the branch as well.
...
# driver a user types and the compiler runtime its JIT loads:
./bazelw build --config=build-mojo //Mojo/tools/mojo:mojo //Mojo:CompilerRT
```
...
| F12   | `lib/KGENToLLVM/LowerPOPToLLVMExternalCalls.cpp`                                                                                                                                                                                             | `external_call` to a symbol _defined_ in the same module. The attribute check reconciles two declarations of an external function, where a disagreement means the call sites disagree about its ABI; a definition is not a declaration to reconcile against, and carries the target attributes any Mojo definition carries. Reaching a compiler-synthesized accessor is exactly this case. The signature check is unchanged.      |
| F13   | `include/Mojo/QuantumKernels/`, `lib/QuantumKernels/`, `tools/kgen/kgen.cpp`, `tools/mojo/Build/mojo-build.cpp`, `tools/mojo/Run/mojo-run.cpp`, `Mojo/BUILD.bazel`, `tools/{kgen,mojo}/BUILD.bazel`                                          | The handoff runs in every driver, not only in `kgen`. F9's code moved out of `tools/kgen/` into a library all three depend on, and `mojo build` / `mojo run` call it at the same point in their pipelines. Adds `resolveQCCPath` and `noQCCMessage`, so where qcc is found and what is said when it is not do not depend on which command was typed. Also rejects an exported kernel name that is not spellable as an identifier. |
...
`//Mojo/test/mojo-integration/...`, which is what covers F12's lowering:
238/240, the two failures being `mojo_build_{object,shared_lib}` calling
`file(1)`, which is not installed in this container.
