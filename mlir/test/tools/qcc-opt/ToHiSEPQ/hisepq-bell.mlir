...
// instructions in total. The point is that one `hisepq` op is one instruction over eight lanes,
...
// One `qv.h` over all eight controls.
// CHECK:         %[[H:.*]] = llvm.intr.vector.insert %[[CTRL_IDX]], %{{.*}}[0] : vector<8xi8> into vector<[8]xi8>
...
// The control lanes <0, 1, ..., 7> are cheap enough that the backend recognizes them as a
// vector index sequence. Nothing materializes for the poison destination of the
// `llvm.intr.vector.insert`: only the eight live lanes get defined.
// CHECK-ASM:       vsetivli {{.*}}, 8, e8, mf2, ta, ma
// CHECK-ASM:       vid.v [[CTRLS:v[0-9]+]]

// `vl` = 8 at LMUL 1, matching the `vector<[8]xi8>` the pass picked for eight lanes.
// CHECK-ASM:       li [[VL:a[0-9]+]], 8
// CHECK-ASM:       vsetvli zero, [[VL]], e8, m1, ta, ma

// One instruction per `hisepq` op, each over all eight lanes.
// CHECK-ASM:       qv.h [[CTRLS]], zero, 0

// The target lanes are the control lanes plus eight, so they fold into a single `vadd.vi`.
