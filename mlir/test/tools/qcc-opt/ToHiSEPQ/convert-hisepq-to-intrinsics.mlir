...
// CHECK-DAG:     %[[VL:.*]] = llvm.mlir.constant(2 : i32) : i32
// CHECK-DAG:     %[[ZERO:.*]] = llvm.mlir.constant(0 : i32) : i32

// Create and insert a vector of two qubits at the front of a scalable vector `vector<[N]xi8>`. Indices 2 and above
// contain poison values, meaning that using those in a computation is undefined behavior - right for us.
// CHECK-DAG:     %[[IDX:.*]] = llvm.mlir.constant(dense<[0, 1]> : vector<2xi8>) : vector<2xi8>
// CHECK-DAG:     %[[POISON:.*]] = llvm.mlir.poison : vector<[8]xi8>
// CHECK:         %[[V0:.*]] = llvm.intr.vector.insert %[[IDX]], %[[POISON]][0] : vector<2xi8> into vector<[8]xi8>

// CHECK:         llvm.call_intrinsic "llvm.riscv.qv.h"(%[[V0]], %[[ZERO]], %[[ZERO]], %[[VL]])
// CHECK:         %[[V1:.*]] = llvm.intr.vector.insert %[[IDX]], %[[POISON]][0] : vector<2xi8> into vector<[8]xi8>
...
// Lane indices need not be contiguous, and need not start at zero.

// CHECK-LABEL: func.func @sparse_lanes
func.func @sparse_lanes() {
...
// Eight lanes still fit LMUL 1 (i.e. `vector<[8]xi8>`).
...
// Nine lanes need LMUL 2 to fit (`vector<[16]xi8>`).
