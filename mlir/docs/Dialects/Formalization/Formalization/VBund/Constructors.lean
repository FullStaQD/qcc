...
/- The algebraic instances on `(cartProd E E').fiber` are found at use sites
via the `abbrev` reduction; declaring them explicitly can trigger kernel
mismatches with the product-type instance chain. -/

end cartProdDef

/-- The unit of the Cartesian product: the zero bundle over the singleton. -/
def cartProdUnit : VBund := zeroBundle PUnit
...
/- The `AddCommGroup`, `Module ℂ`, and `FiniteDimensional ℂ` instances on
`(linProd E E').fiber` are found at use sites via the `abbrev` reduction;
declaring them explicitly triggers a kernel mismatch with the
`TensorProduct` instance chain. -/

end linProdDef

/-- The unit of the linear product: ℂ over the singleton. -/
def linProdUnit : VBund := haloBundle PUnit
...
/- The algebraic instances on `(coprod E E').fiber` are found at use sites
via the `abbrev` reduction. -/

end coprodDef

/-- The initial object of the coproduct: the empty bundle. -/
abbrev coprodInit : VBund :=
  { base := Empty, fiber := fun w => Empty.elim w }

instance : Fintype (coprodInit).base := inferInstanceAs (Fintype Empty)
/- The algebraic instances on `coprodInit.fiber` are vacuous (the base is
`Empty`); they are found at use sites via the `abbrev` reduction. -/
