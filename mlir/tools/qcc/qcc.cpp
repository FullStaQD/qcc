...
int main(int argc, char** argv) {
  ...
  const cl::opt<std::string> mattr("mattr", cl::desc("Target features, comma-separated (see --list-targets)"),
                                   cl::value_desc("+feature,..."), cl::cat(qccCategory));
  ...
  const mlir::FailureOr<llvm::SmallVector<llvm::StringRef>> features = qcc::parseFeatures(*target, mattr);
  ...
  qcc::buildPipeline(pm, target, *features);
